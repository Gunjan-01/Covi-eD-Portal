...
exports.getCourse = asyncHandler(async (req, res, next) => {
    const course = await(await Course.findById(req.params.id)).populate({path: 'Bootcamp', select: 'name description'});

    if (! course) {
        return next(new ErrorResponse(`no course with givesn id ${
            req.params.id
        } exist`), 404);
    }
...
exports.addCourse = asyncHandler(async (req, res, next) => {
    ...
    if (! bootcamp) {
        return next(new ErrorResponse(`no bootcamp with givesn id ${
